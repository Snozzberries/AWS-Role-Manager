...
$bucket = "awslambdapowershelldemo"
...
"@
    ...
    if($policyName -eq "managed")
    {
        ...
        $file = Invoke-WebRequest (Get-S3PreSignedURL -BucketName $bucket -Expire (Get-Date).AddMinutes(1) -Protocol HTTP -Key "$entity/$roleName/$policyName.json")
